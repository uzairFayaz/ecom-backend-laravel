        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <h1>Edit Category: {{ $category->category_name }}</h1> <!-- Changed to 'category_name' -->
                    @if (!Auth::check())
                        <p>Please <a href="{{ route('login') }}" class="text-indigo-600 hover:text-indigo-800">log in</a> to edit categories.</p>
                    @else
                        <form action="{{ route('admin.categories.update', $category) }}" method="POST">
                            ...
                            <div class="mb-4">
                                ...
                                <select name="parent_cat_id" id="parent_cat_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                                    ...
                                    @foreach ($categories as $cat)
                                        <option value="{{ $cat->id }}" {{ old('parent_cat_id', $category->parent_cat_id) == $cat->id ? 'selected' : '' }}>{{ $cat->category_name }}</option>
                                    @endforeach
                                </select>
                                @error('parent_cat_id')
                                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                            ...
                            <div class="mb-4">
                                ...
                                <select name="status" id="status" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                                    <option value="" {{ old('status', $category->status) == '' ? 'selected' : '' }}>Select Status</option>
