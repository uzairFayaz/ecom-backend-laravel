        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    @if (!Auth::check())
                        <p>Please <a href="{{ route('login') }}" class="text-indigo-600 hover:text-indigo-800">log in</a> to create a product.</p>
                    @else
                        <form action="{{ route('admin.products.store') }}" method="POST">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="number" name="price" id="price" step="0.01" value="{{ old('price') }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">
                                @error('price')
                                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <button type="submit" class="bg-indigo-600 text-white px-4 py-2 rounded">Create Product</button>
